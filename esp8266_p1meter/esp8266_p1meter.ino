...
bool mqtt_reconnect()
{
    ...
    while (!mqtt_client.connected() && MQTT_RECONNECT_RETRIES < MQTT_MAX_RECONNECT_TRIES)
    {
        ...
        // * Attempt to connect
        if (mqtt_client.connect(ESP_NAME, MQTT_USER, MQTT_PASS))
        {
            Serial.println(F("MQTT connected!"));

            // * Once connected, publish an announcement...
            char *message = new char[16 + strlen(ESP_NAME) + 1];
            strcpy(message, "p1 meter alive: ");
            strcat(message, ESP_NAME);
...
void setup_mdns()
{
    ...
    bool mdns_result = MDNS.begin(ESP_NAME);
    if (mdns_result)
    {
...
void setup()
{
    ...
    WiFiManagerParameter CUSTOM_MQTT_HOST("host", "MQTT ESP_NAME", MQTT_HOST, 64);
    ...
    if (!wifiManager.autoConnect("esp1meter"))
    {
